...
from robovlms.data.openvla_datasets.rlds.utils.cot_utils import (
    get_cot_tags_list,
    get_cot_database_keys,
)
from robovlms.data.openvla_datasets.overwatch import initialize_overwatch
from robovlms.data.openvla_datasets.rlds import obs_transforms, traj_transforms
from robovlms.data.openvla_datasets.rlds.utils import goal_relabeling, task_augmentation
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    reasoning_dataset_filename: str = "embodied_features_bridge.json",
    use_cot: bool = False,
    ...
    Args:
        ...
        reasoning_dataset_filename (str): the filename of the reasoning dataset.
        use_cot (bool): whether to use the reasoning dataset.
    Returns:
    ...
    reasoning_dataset_path = os.path.join(data_dir, name, reasoning_dataset_filename)

    def build_reasoning_dict(raw_dict):
        ...
        def reasoning_dict_to_str(d):
            ...
            reasoning_parts = [
                (database_keys[tag], d.get(database_keys[tag], "")) for tag in tags
            ]
        ...
        for file_name in tqdm(list(raw_dict.keys())):
            for episode_id in raw_dict[file_name].keys():
                ...
                for i in raw_dict[file_name][episode_id]["reasoning"].keys():
                    ...
                    if "next_gripper_position" in trajectory_features.keys():
                        reasoning_dict["gripper"] = str(
                            trajectory_features["gripper_position"][int(i)]
                        )
                        reasoning_dict["next_gripper"] = str(
                            trajectory_features["next_gripper_position"][int(i)]
                        )

                    elif "gripper_position" in trajectory_features.keys():
                        gripper_lookahead_n = (
                            5  # list this many future positions of the gripper
                        )
                        if trajectory_features["gripper_position"] is not None:
                            if (
                                0
                                <= int(i)
                                < len(trajectory_features["gripper_position"])
                            ):
                                future_positions = []
                                for j in range(gripper_lookahead_n):
                                    if int(i) + j < len(
                                        trajectory_features["gripper_position"]
                                    ):
                                        future_positions += trajectory_features[
                                            "gripper_position"
                                        ][int(i) + j]
                                    else:
                            ...
                            if 0 <= int(i) < len(trajectory_features["bboxes"]):
                                if len(trajectory_features["bboxes"][int(i)]) > 0:
                                    ...
                                    reasoning_dict["bboxes"] = ", ".join(
                                        [
                                            f"{name} {box!s}"
                                            for prob, name, box in boxes_list
                                        ]
                                    )
    ...
    if use_cot:
        prebuilt_reasoning_dataset_path = os.path.join(
            data_dir, name, reasoning_dataset_filename.replace(".json", "_built.pkl")
        )
        if os.path.exists(prebuilt_reasoning_dataset_path):
            print("Loading prebuilt reasoning dataset...")
            with open(prebuilt_reasoning_dataset_path, "rb") as f:
                reasoning_keys, reasoning_values = pickle.load(f)
        else:
            with open(reasoning_dataset_path, "r") as f:
                reasoning_dataset = json.load(f)
            reasoning_keys, reasoning_values = build_reasoning_dict(reasoning_dataset)
            with open(prebuilt_reasoning_dataset_path, "wb") as f:
                pickle.dump((reasoning_keys, reasoning_values), f)

        reasoning_dataset = tf.lookup.StaticHashTable(
            tf.lookup.KeyValueTensorInitializer(reasoning_keys, reasoning_values),
            default_value="",
        )
    ...
    def restructure(traj):
        ...
        if "libero" in name:
            # NOTE(zbzhu): In LIBERO, multiple episodes in the same file actually assigned different file_path, which makes episode_id not used and all set to 0
            step_ids = episode_ids + "_0_" + indices
        else:
            step_ids = file_names + "_" + episode_ids + "_" + indices

        if use_cot:
            reasonings = reasoning_dataset.lookup(step_ids)
    ...
    *,
    ...
    if filter_langs:
        ...
        def language_filter(lang):
            ...
            # lang = x["task"]["language_instruction"].numpy()[0].decode()
            # lang = '_'.join(lang.lower().split())

            # if ('pick' in lang or 'place' in lang) and 'coke can' in lang:
            #     return True
            ...
            # if 'eggplant' in lang and 'put' in lang:
            #     return True
            # if 'cube' in lang and 'on top of' in lang:
            #     return True
            # if ('on plate' in lang or 'on towel' in lang) and ('carrot' in lang or 'spoon' in lang):
            #     return True
            # if ('basket' in lang or 'sink' in lang) and 'put' in lang and 'eggplant' in lang:
            #     return True

            return False

        lang_list_bridge = json.load(
            open(
                "/mnt/bn/robotics-data-lxh-lq/tmp/RobotVLM/dataset_stats/bridge_orig.json",
                "r",
            )
        )
        lang_list_rt = {}

        lang_list_rt = json.load(
            open(
                "/mnt/bn/robotics-data-lxh-lq/tmp/RobotVLM/dataset_stats/fractal20220817_data.json",
                "r",
            )
        )
        lang_list_bridge = {}
        ...
        def tf_lang_filter(x):
            ...
            for lang in lang_list:
                if flag is None:
                    ...
                    flag = tf.math.logical_or(
                        flag,
                        tf.math.reduce_any(x["task"]["language_instruction"] == lang),
                    )
            return flag
    ...
    if train and subsample_length is not None:
        ...
        if filter_langs:
            ...
            def language_filter(x):
                ...
                if ("pick" in lang or "place" in lang) and "coke can" in lang:
                    return tf.ones(1, dtype=tf.bool)
                if "move" in lang and "near" in lang:
                    return tf.ones(1, dtype=tf.bool)
                if ("open" in lang or "close" in lang) and "drawer" in lang:
                    return tf.ones(1, dtype=tf.bool)
                ...
                if "cube" in lang and "on top of" in lang:
                    return tf.ones(1, dtype=tf.bool)
                if "on plate" in lang and ("carrot" in lang or "spoon" in lang):
                    return tf.ones(1, dtype=tf.bool)
                if "basket" in lang:
                    return tf.ones(1, dtype=tf.bool)

                return tf.zeros(1, dtype=tf.bool)

            dataset = dataset.filter(lambda x: language_filter(x))
    ...
    for dataset_kwargs in dataset_kwargs_list:
        ...
        _, dataset_statistics = make_dataset_from_rlds(
            **data_kwargs,
            train=train,
            use_cot=use_cot,
            reasoning_dataset_filename=cot_file_name,
        )
