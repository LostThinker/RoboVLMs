...
def chunk_act_obs(
    ...
    traj["chunk_mask"] = (action_chunk_indices >= 0) & (
...
def new_chunk_act_obs(
    ...
    if window_sample == "range":
        ...
        left_range_mask = ~tf.sequence_mask(
            left_range, window_size + future_action_window_size
        )
...
def chunk_as_episode(traj: Dict, frame_num: int) -> Dict:
    traj_len = tf.shape(traj["action"])[0]
    indices = tf.cast(
        tf.linspace(0.0, tf.cast(traj_len - 1, tf.float32), frame_num), tf.int32
    )
