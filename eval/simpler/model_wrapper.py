...
class BaseModelInference(CustomModel):
    def __init__(
        ...
        if no_action_ensemble:
            raise NotImplementedError(
                "No action ensemble is currently not supported for simpler models"
            )
        ...
        self.init_config(ckpt_path, configs, device, save_dir, raw_calvin=True)
        self.update_plan_freq = update_plan_freq
    ...
    def step(self, image, goal):
        ...
        if self.use_cot:
            ...
            self.prev_thought = thought.copy()
        else:
        ...
        if self.use_cot:
            return raw_action, env_action, thought
        else:
